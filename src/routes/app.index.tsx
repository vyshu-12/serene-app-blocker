...
function AppsFolder() {
  ...
  setOpenApp(null);
  toast.error(`⛔ ${app.name} is blocked. Come tomorrow.`);
   return;
   }
    setOpenApp(app);
  }
...
function AppSessionDialog({
 ...
 useEffect(() => {
  ...
  const id = window.setInterval(() => {
    setUsed((u) => {
      if (u >= limit) return u; // 🔥 THIS LINE FIXES YOUR BUG
      const next = u + 1;
      onTick(next);
      return next;
    });
  ...
  // force final limit (important fix)
  if (timer && user) {
    supabase
      .from("app_timers")
      .update({
        used_seconds: timer.limit_seconds,
      })
      .eq("user_id", user.id)
      .eq("app_key", app.key);
  }
